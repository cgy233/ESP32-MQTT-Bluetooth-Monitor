...
static void handle_bt_device_result(struct disc_res_param *disc_res)
{
    ...
    if (cod->major == ESP_BT_COD_MAJOR_DEV_PHONE || (find_scan_result(disc_res->bda, bt_scan_results) != NULL)) {
...
static void handle_ble_device_result(struct ble_scan_result_evt_param *scan_rst)
{
    ...
    if (scan_rst->ble_addr_type != BLE_ADDR_TYPE_PUBLIC || !adv_name_len)
    {
        return;
    }
    ...
    memcpy(device.mac, scan_rst->bda, sizeof(device.mac));
    // device.confidence = calculate_confidence(r); // You need to implement this function
    device.rssi = scan_rst->rssi;

    for (int i = 0; i < DEVICE_NUMBER; i++) {

        if (memcmp(ble_tracker_data.devices[i].mac, device.mac, sizeof(device.mac)) == 0) {
            ble_tracker_data.devices[i].rssi = device.rssi;
            strncpy(ble_tracker_data.devices[i].name, device.name, sizeof(device.name));
            if (ble_tracker_data.devices[i].confidence < 100)
            {
                ble_tracker_data.scan_state = 1;
            }
            break;
        }
    }

    xQueueSend(ble_device_queue, &ble_tracker_data, portMAX_DELAY);
